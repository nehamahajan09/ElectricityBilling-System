...
public class CalculateBill extends JFrame implements ActionListener{
    ...
    public void actionPerformed(ActionEvent ae){
        if(ae.getSource() == b1){
            ...
            try{
                Conn c = new Conn();
                ResultSet rs = c.s.executeQuery("select * from taxx");
                while(rs.next()){
                    ...
                    total_bill += Integer.parseInt(rs.getString("service_taxx"));
                    total_bill += Integer.parseInt(rs.getString("swacch_bharat_cess"));
                    total_bill += Integer.parseInt(rs.getString("fixed_taxx"));
                }
            }catch(Exception e){}

            String q = "insert into bill values('"+meter_no+"','"+month+"','"+total_bill+"','"+units+"', 'Not Paid')";

            try{
                Conn c99 = new Conn();
                c99.s.executeUpdate(q);
